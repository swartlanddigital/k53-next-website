...
export default function Page() {
  return <><Header/><main id="main"><PageHero eyebrow="USE IT WELL" title="Clear terms. No surprise turns." lede="The rules for using the K53 Next app, website and study resources."/><article className="content-page"><div className="shell narrow prose">
    ...
    <p>Native Android Premium is a once-off purchase made through Google Play, with no monthly subscription. The Google Play checkout shows the current price before you buy. Premium provides unlimited practice, full practice sets and removal of adverts in the native Android app while the service is available. It does not guarantee an official test result.</p>
    <p>Sign in before buying. The purchase is linked to the K53 Next account used at checkout. To restore it, use the same Google Play purchasing account and the same K53 Next account, then select Restore Premium. A pending or declined payment does not activate Premium. A refunded or reversed purchase can remove Premium access.</p>
    <p>Google Play handles payment and applicable refund requests for Android purchases. Deleting your K53 Next account does not automatically refund a purchase. Nothing in these terms limits rights that cannot lawfully be excluded. Web payments continue through Yoco and are separate from native Android purchases.</p>
